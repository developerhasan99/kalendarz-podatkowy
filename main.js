$(function () {
	...
	function renderDate() {
		...
		for (let i = 0; i < daysInMonth; i++) {
			let item =
				'<div class="calendar__day inside ' +
				activeCurrentDay(i + 1) +
				" " +
				isHavingEvent(i + 1 + "." + (currentMonth + 1) + "." + currentYear) +
				'">' +
				(i + 1) +
				"</div>";

			allDayInMonth.push(item);
		}
	...
	function renderEvent(date) {
		...
		allEvents.forEach(function (element) {
			...
			if (
				new Date(elementTimeStamp).getTime() ===
		...
		if (matchEvent == null) {
			$eventContainer.append(
				'<div class="calendar__info"><p>' + noEventMessage + "</p></div>"
			);
		} else {
			let dom = "";
			dom += '<div class="calendar__info"><p>' + matchEvent.day + "</p></div>";
			matchEvent.events.forEach(function (element) {
	...
	$dateContainer.on("click", function (event) {
		const $targetElement = $calendar.find(event.target);
		const $allInsideDay = $calendar.find(".calendar__day.inside ");
		if (
			$targetElement.hasClass("inside") &&
			!$targetElement.hasClass("active")
		) {
			$allInsideDay.removeClass("active");
			$targetElement.addClass("active");
			let selectedDate =
				$targetElement.text() + "." + (currentMonth + 1) + "." + currentYear;
